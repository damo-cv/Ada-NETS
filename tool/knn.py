...
__all__ = [
    ...
    'knns2ordered_nbrs', 'fast_knns2spmat_adaptivek'
]
...
def fast_knns2spmat_adaptivek(knns, k_arr, th_sim=0.7):
    # convert knns to symmetric sparse matrix
    from scipy.sparse import csr_matrix
    eps = 1e-5
    n = len(knns)
    ...
    row, col = np.where(sims >= th_sim)  # 这里划分阈值了
    new_row, new_col = [], []
    for row_idx, col_idx in zip(row, col):
        thresK = k_arr[row_idx]
        if col_idx >= thresK:
            continue
        new_row.append(row_idx)
        new_col.append(col_idx)
    row, col = np.array(new_row), np.array(new_col)
    ...
    col = col[idxs]
    data = sims[row, col]
    col = nbrs[row, col]  # convert to absolute column of the FULL N*N adj matrix
    assert len(row) == len(col) == len(data)
    spmat = csr_matrix((data, (row, col)), shape=(n, n))
    return spmat
